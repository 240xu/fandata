...
class AnalyzeUrl(
    ...
    private fun handleDataUrl(dataUrl: String) {
        try {
            ...
            if (afterData.startsWith(";base64,")) {
                ...
                // 需要找到 base64 结束位置：base64 内容只包含 [A-Za-z0-9+/=]
                val base64End = encoded.indexOfFirst { ch -> !(ch in 'A'..'Z' || ch in 'a'..'z' || ch in '0'..'9' || ch == '+' || ch == '/' || ch == '=') }
                if (base64End > 0) {
                    base64Part = encoded.substring(0, base64End)
                    suffix = encoded.substring(base64End).trimStart(',').trim()
                } else {
                    base64Part = encoded
                }
            } else {
            ...
            type = "data"
            // 提取 {"type":"gysearch"} 等路由类型
            if (!suffix.isNullOrEmpty() && suffix.startsWith("{")) {
                try {
                    val jsonObj = com.google.gson.Gson().fromJson(suffix, Map::class.java)
                    dataUrlType = jsonObj?.get("type")?.toString()
                    Log.d(TAG, "data URL 路由类型: $dataUrlType")
                } catch (_: Exception) {}
            }
            Log.d(TAG, "data URL 解码成功: ${decoded.take(200)}, type=$dataUrlType")
        } catch (e: Exception) {
    ...
    fun getStrResponse(): StrResponse {
        Log.d(TAG, "getStrResponse ENTRY: type=$type, dataUrlType=$dataUrlType, dataUrlContent=${dataUrlContent?.take(100)}, url=${url.take(100)}")
        // 如果是 data: URL
        if (type == "data") {
            // 聚合路由类型：gysearch / gycontent
            // 将解码后的参数 POST 到聚合 API 对应端点
            if (!dataUrlType.isNullOrEmpty() && dataUrlType != "data") {
                val sourceUrl = source?.bookSourceUrl?.trimEnd('/') ?: ""
                if (sourceUrl.isNotEmpty() && !dataUrlContent.isNullOrEmpty()) {
                    val endpoint = "$sourceUrl/$dataUrlType"
                    Log.d(TAG, "聚合路由: type=$dataUrlType, endpoint=$endpoint, body=${dataUrlContent?.take(200)}")
                    try {
                        val headers = LinkedHashMap(headerMap)
                        headers["Content-Type"] = "application/json; charset=utf-8"
                        val response = httpHelper.post(endpoint, dataUrlContent!!, headers, null)
                        if (response.isSuccessful() && !response.body.isNullOrBlank()) {
                            Log.d(TAG, "聚合路由成功: ${response.body?.take(200)}")
                            return response
                        }
                        Log.w(TAG, "聚合路由响应为空或失败: code=${response.code()}, body=${response.body?.take(100)}")
                    } catch (e: Exception) {
                        Log.e(TAG, "聚合路由请求失败: ${e.message}")
                    }
                }
            }
            // 普通 data URL 或聚合路由失败时，返回解码后的内容
